...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/">
		<html>
			<head>
				...
				<script src="{$workspace}/js/gcse.min.js" type="text/javascript" />
			</head>
			<body>
	...
	<xsl:template name="fecha-formato">
		<xsl:param name="fecha" />
		<xsl:value-of select="concat(substring($fecha/@iso, 9, 2), '/', substring($fecha/@iso, 6, 2), '/', substring($fecha/@iso, 1, 4))" />&#160;<xsl:value-of
			  select="$fecha/@time" />
	</xsl:template>
